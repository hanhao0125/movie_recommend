...
# 对原始数据进行处理
def data_deal():
    # print(pd.read_sql('select * from movie_eva', db.engine))
    ...
    all_ratings = pd.read_sql('select * from movie_eva where score > 8', db.engine)
    all_ratings["Favorable"] = all_ratings["score"] > 8
...
def getFeature(ratings):
    ...
    favorate_reviews_by_users = dict((k, frozenset(v.values)) for k, v in favorate_ratings.groupby("user_id")["movie_id"])
    # 定义每部电影的影迷数
...
def get_movie_name(movie_id):
    movie = m.Movie.query.get(int(movie_id))
    if movie is None:
        return 'none'
    return movie.name
...
# 下面将通过频繁项集来生成关联规则。
def generate_rule(frequent_itemsets, favorate_reviews_by_users):
    candidate_rules = []
...
def get_rule(min_support=150, confidence=0.6):
    ratings = data_deal()
    frequent_itemsets = {}
    ...
    # 生成关联规则
    sorted_confidence = generate_rule(frequent_itemsets, favorate_reviews_by_users)
    return sorted_confidence
...
if __name__ == "__main__":
    ...
    # ratings_test = all_ratings[all_ratings["user_id"].isin(range(201))]
    sorted_confidence_test = get_rule()
